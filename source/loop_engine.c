...
static inline void apply_edge_fade_stereo(float* buf, const uint32_t loop_start,
                                          const uint32_t loop_samples, uint32_t fade_samples)
{
...
{
  float bars_f = (self && self->ports.bars) ? *(self->ports.bars) : (float)DEFAULT_NUM_BARS;
  if (bars_f < 1.0f)
  {
    bars_f = 1.0f;
  }
...
static bool compute_transport_phase_index(const Alo* self, double global_beats,
...
static void update_transport_beats(Alo* self, double global_beats)
{
  ...
  {
    /* Host transport jumped backwards (common on stop/rewind).
     * Do NOT clear committed loop audio; only resync transport/UI state.
     */
    clear_inflight_actions_and_sync_controls(self);
    request_ui_cycle_resync(self);

    /* Stop any slice one-shots immediately to avoid hanging notes across a locate. */
    alo_slice_sampler_reset(&self->slice_sampler);

    if (any_audio)
    {
      /* Restart loop phase from the new transport position so playback resumes
       * cleanly on the next running block.
       */
    ...
    {
      /* Preserve the special first-loop quantization semantics. */
      self->have_loop_origin  = false;
      self->loop_origin_beats = 0.0;
    }
  }
...
static void update_position_from_atom(Alo* self, const LV2_Atom_Object* obj)
{
  ...
  if (beat && beat->type == uris->atom_Float)
  {
    ...
    {
      const double global_beats = (double)bar_index * (double)self->bpb + (double)bar_beat2;
...
void run_clicks(Alo* self, uint32_t n_samples)
{
  ...
  const float delta_beats =
      (float)((samples_per_beat > 1e-9) ? ((double)n_samples / samples_per_beat) : 0.0);
...
void run_loops(Alo* self, uint32_t n_samples)
{
  ...
  if (sampler_gain < -2.5f) sampler_gain = -2.5f;
  if (sampler_gain > 2.5f) sampler_gain = 2.5f;
  ...
  const uint32_t bars_i = get_bars_i(self);
  uint32_t       bar_len =
      (bars_i > 0 && self->loop_samples > 0) ? (self->loop_samples / bars_i) : self->loop_samples;
  if (!bar_len)
  {
  ...
  while (block_base < n_samples)
  {
    ...
    if (sampler_enabled && sampler_gain > 0.0f && sampler_busy)
    {
      ...
      if (self->ports.slices_per_bar)
      {
        float slices = *(self->ports.slices_per_bar);
        if (slices > 1.0f)
        {
          slice_gain_scale = 1.0f / sqrtf(slices);
        }
      }
      alo_slice_sampler_process_chunk(&self->slice_sampler, self, sampler_track_gain, block_base,
                                      blk, slice_l_s, slice_r_s);
      for (uint32_t pos = 0; pos < blk; ++pos)
      {
    ...
    for (uint32_t pos = 0; pos < blk; ++pos)
    {
      ...
      for (int t = 0; t < NUM_TRACKS; ++t)
      {
        ...
        if (self->track_state[t] == TRACK_REC_BASE)
        {
          ...
          {
            self->track_state[t] = TRACK_IDLE;
            apply_edge_fade_stereo(self->loop_buf[t], self->loop_start, self->loop_samples,
        ...
        else if (self->track_state[t] == TRACK_REC_OVERDUB)
        {
          ...
          if (self->rec_remaining_samples[t] == 0)
          {
            ...
            if (self->od_count[t] < ALO_MAX_UNDO_LAYERS)
            {
              ...
              if (layer < ALO_MAX_UNDO_LAYERS && self->od_buf[t][layer])
              {
                apply_edge_fade_stereo(self->od_buf[t][layer], self->loop_start, self->loop_samples,
                                       edge_fade_samples_u32(self));
              }
              self->od_count[t]++;
            }
