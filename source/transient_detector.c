...
static float
td_safe_time_ms(float v, float fallback)
{
    return (v > 0.0f) ? v : fallback;
}

static uint32_t
td_ms_to_samples(float sample_rate, float ms)
{
...
static void
td_reset_candidate(TransientCandidate* c)
{
    if (!c) {
        return;
    }
...
static float
td_compute_ratio(float fast_env, float slow_env)
{
...
static void
td_begin_candidate(TransientDetector* td, uint32_t sample_index, float amplitude, float ratio)
{
...
static void
td_update_candidate_peak(TransientCandidate* c, uint32_t sample_index, float amplitude, float ratio)
{
    if (!c || !c->active) {
        return;
    }
...
static bool
td_candidate_long_enough(const TransientDetector* td, const TransientCandidate* c)
{
    if (!td || !c || !c->active) {
        return false;
    }

    const uint32_t dur = (c->end_sample >= c->start_sample)
        ? (c->end_sample - c->start_sample + 1u)
        : 0u;
...
static void
td_finish_candidate(TransientDetector* td, bool* candidate_complete_out)
{
...
void
td_init_ex(TransientDetector *td,
           float sample_rate,
           ...
           float end_ratio)
{
...

    {
        ...
        td->debounce_samples = td_ms_to_samples(sr, td_safe_time_ms(debounce_time_ms, TD_DEFAULT_DEBOUNCE_MS));
        td->min_burst_samples = td_ms_to_samples(sr, td_safe_time_ms(min_burst_time_ms, TD_DEFAULT_MIN_BURST_MS));
    }
        ...
        float debounce_time_ms)
{
    td_init_ex(td,
               ...
               threshold_ratio,
               debounce_time_ms,
               TD_DEFAULT_MIN_BURST_MS,
               TD_DEFAULT_END_RATIO);
}

TransientDetectorEvent
td_process_sample_ex(TransientDetector *td, float sample)
{
...
bool
td_process_sample(TransientDetector *td, float sample)
{
    return td_process_sample_ex(td, sample).triggered;
}

const TransientCandidate*
td_last_completed_candidate(const TransientDetector* td)
{
    return td ? &td->last_completed_candidate : (const TransientCandidate*)0;
}

const TransientCandidate*
td_current_candidate(const TransientDetector* td)
{
    return td ? &td->candidate : (const TransientCandidate*)0;
}

float
td_current_onset_ratio(const TransientDetector* td)
{
    return td ? td->current_ratio : 0.0f;
}

float
td_current_amplitude(const TransientDetector* td)
{
